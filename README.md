...
- 開放資料平台 `service.taipower.com.tw` 沒有這幾支曲線檔
  （2026-08-06 實測 d006002~d006008、d006021 全部 404）

  ★★ **2026-08-29 更正**：當時把僅存的 `d006001`「各機組發電量」記成
  「內容不重疊」，那是錯的。它逐機組帶「機組類型」，**按類型加總之後與
  `loadfueltype.csv` 的 12 欄逐類完全吻合**（實測三輪，最大差 0.5 MW＝
  四捨五入的上界）。不重疊的是**形狀**不是內容：它是單一時點的快照、
  不是整天的曲線，所以取代不了 CSV——但正好可以拿來**按名字驗欄位對應**，
  現在每次跑都抓（見 CLAUDE.md「欄位對應」）。
- 同平台的 `d006009`／`d006010`（區域間潮流／逐機組歷史曲線）是**季度歷史檔**：
  189 MB、落後約四個月，而且口徑不同（只有台電自有機組、沒有民營電廠，
  類別只有 10 種）。**刻意不抓**——中繼那台 Mac 有流量限制，而它根本不合用。
...
五支檔。前四支的 Base 是
`https://www.taipower.com.tw/d006/loadGraph/loadGraph/data/`，
第五支在開放資料平台（見上）。
...
★ 平台紀律：**原文全存，解析錯了可以重跑。** 每次執行把五支檔的原始 bytes
